...
public class Grid_Behavior {
  ...
  public static int call_getRowCount_1213877333382(SNode thisNode) {
    return ListSequence.<SNode>fromList(SLinkOperations.getTargets(thisNode, "row", true)).count();
  }
  ...
  public static int call_getColumnsCount_1213877333391(SNode thisNode) {
    ...
    for (SNode row : SLinkOperations.getTargets(thisNode, "row", true)) {
      columnsCount = Math.max(columnsCount, ListSequence.<SNode>fromList(SLinkOperations.getTargets(row, "component", true)).count());
    }
    return columnsCount;
  }
}
